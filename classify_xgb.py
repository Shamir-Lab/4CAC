...
def main(args):
    ...
    else: outfile = infile + '.probs_xgb_4class.out'
    n_procs = args.num_processes

    c = classifier.xgbClass(n_procs)
    seq_names = []
    seqs = []
    ...
    with open(outfile,'w') as o:
        o.write('header' + '\t' + 'viral_score' + '\t' + 'plas_score' + '\t' + 'prokar_score' + '\t' + 'eukar_score' + '\n')
        for name, seq, _ in utils.readfq(fp):
            ...
            if i % 100000 == 0:
                ...
                for j,p in enumerate(probs):
                    o.write(seq_names[j] + '\t' + str(p[0]) + '\t' + str(p[1]) + '\t' + str(p[2]) + '\t' + str(p[3]) + '\n')
                seq_names = []
                seqs = []
